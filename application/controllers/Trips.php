...
function pricingForm()
	  ...
	  $id=$this->uri->segment(3);//$this->session->userdata('id');//$this->uri->segment(4);
	   if($this->session->userdata('id'))
             ...
             $data['cities'] = $this->users->tblLIst('cities','countryId',$countries[0]['countryId'],'cityStatus=1');
...
//
	 ...
	 $WaitingperHour=$this->input->post('WaitingperHour', TRUE);
                 $data =  array('packageId' => $packageId
                    ...
                    $info= $this->users->getPeicingData($packageId,$countryId,$cityId);
                 if(count($info)>0){
                     $this->session->set_flashdata('pricingExist', '1');
													  $this->session->set_flashdata('email', $packageId);
													  redirect(site_url('Trips/pricingForm'));
                 }
             }
...
//
             ...
             $driverCost=$this->input->post('driverCost_'.$i, TRUE);
           //  echo count($levelId);
     ...
     $sts=$this->uri->segment(4);
			 if($this->session->userdata('id'))
			   {			
						if($id )
						   {
                            	$pricing = $this->users->Details($id);
                              $this->db->where('packageId', $pricing['packageId']);
                 $this->db->where('countryId', $pricing['countryId']);
                 $this->db->where('cityId', $pricing['cityId']);
						   $this->db->delete('prices');
	   ...
	   if($this->session->userdata('id'))
			   {
			       ...
			       $data['type'] = 1;
				   $data['pages'] = $this->users->pricingList('a.packageId,a.countryId,a.cityId');//$this->users->tblLIst('		prices a INNER JOIN packages b ON a.packageId = b.packageId INNER JOIN countries c ON a.countryId = c.countryId INNER JOIN cities d ON a.cityId = d.cityId INNER JOIN levels e ON a.levelId=e.levelId INNER JOIN tripTypes f ON a.typeId=f.typeId','','','a.packageId,a.countryId,a.cityId');	
  ...
  function Saveoffer()
    {
	 ...
	 $offerDetails=$this->input->post('offerDetails');
//
	  $countryId=$this->input->post('countryId', TRUE);
	 $cityId=$this->input->post('cityId', TRUE);
 ...
 function Deleteoffer()
  ...
  function  offerList()
	  ...
	  if($this->session->userdata('id'))
			   {
			       $data['type'] = 1;
				   $data['pages'] = $this->users->getOffers();//$this->users->tblLIst('offers a   LEFT JOIN countries b ON a.countryId = b.countryId  LEFT JOIN cities c ON c.cityId = a.cityId  LEFT JOIN levels d ON a.levelId = d.levelId');	
