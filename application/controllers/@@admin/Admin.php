...
function __construct()
	...
	{
		header('Content-Type: application/json');
		//$data = $this->db->where('passengerMobile',$this->input->post('countryTel').$this->input->post('passengerMobile'))->get('passengers')->row_array();
		$data = $this->db->where('passengerMobile',$this->input->post('passengerMobile'))
					->where('passengerCountryId',$this->input->post('passengerCountryId'))
		   ...
		   if($this->session->userdata('id'))
			   {
					...
					/*  $data['open_orders'] = $this->admin->get_open_orders();
					  ...
					  $data['levels']=$this->db->get('levels')->result();
					  $data['MAIN'] = "MAIN";	
 ...
 //===========================
   function filemanager()
    ...
    function switchLanguage($language = "") {
      ...
      /*  echo "<script>console.log('".$language."');document.location.reload();</script>";*/
    }
 //===========================
   function SignOut()
     {
						  ...
						  if($password && $username) 
							{
							 ...
							 $data['user'] = $this->admin->checkUser($username,$password);
							   ...
							   if(isset($data['user']['userId']) && $data['user']['userId'])
								  {
...
//echo $phone; exit;
			if (valid_email($email))
			{
						...
						redirect(base_url('').'index.php/admin/Admin/Settings/1'); 
				  
			} else {
			
			  redirect(base_url('').'index.php/admin/Admin/Settings/2');
