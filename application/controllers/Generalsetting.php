 ...
 //****************************************//
    function Setting()
    {
		...
		{			     
 						...
 						$data['page'] = $this->generalsetting->All('settings');
 						$data['pageTitle']=lang('Setting');
 				   
				    
 				  $this->template->load('admin/Container', 'admin/generalsetting/setting/page_form',$data); 
    ...
    function SaveSetting()
    {
		...
		{			     
                   ...
                   {
                       $id=$i+1;
                       $data =  array('value' => $values[$i]);  
                        $this->db->where('id',$id);
						     $this->db->update('settings', $data);
                   }
				    
 				  redirect(base_url('generalsetting/Setting')); 
...
function DeleteBrand()
     {
		...
		if( $this->session->userdata('id') &&  $this->session->userdata('id') !="")
		{
						...
						if($page_id)
						   {
                            $brandStatus=$this->uri->segment(4);
...
function AllModel()
     {
		...
		if( $this->session->userdata('id') &&  $this->session->userdata('id') !="")
		{
						$data['pages'] = $this->generalsetting->get_Models();
...
function DeleteModel()
     {
		...
		if( $this->session->userdata('id') &&  $this->session->userdata('id') !="")
		{
						...
						if($page_id)
						   { $modelStatus=$this->uri->segment(4);
...
function AllLevel()
     {
		...
		{					 
						...
						$data['pageTitle']=lang('All_levels');
...
function DeleteLevel()
     {
		...
		if( $this->session->userdata('id') &&  $this->session->userdata('id') !="")
		{
						...
						if($page_id)
						   {
                             $levelStatus=$this->uri->segment(4);
...
//#############################cats###############
  //#############################Country###############
	...
	$packageName_en=$this->input->post('packageName_en', TRUE);
	 ...
	 if(!$packageStatus)$packageStatus=0;
...
function Deletepackage()
	...
	if($this->session->userdata('id'))
	{
						$page_id=$this->uri->segment(3);
						   ...
						   $countryStatus=$this->uri->segment(4);
 ...
 //#############################Country###############
	...
	function SaveCountry()
	 ...
	 if(!$countryStatus)$countryStatus=0;
 ...
 //#############################City###############
    function CityForm()
      ...
      //  echo 'ID='.$id;
		...
		{
            if(isset($cnID))
				 $data['countries'] = $this->generalsetting->All('countries','countryId','countryId',$cnID,'countryStatus =1');
            else $data['countries'] = $this->generalsetting->All('countries','countryId','countryStatus',1);
				  if(isset($id) && $id!='saved') 
				   {    
	...
	function SaveCity()
	 ...
	 $driverCost=$this->input->post('driverCost', TRUE);
	/* $link=$this->input->post('link', TRUE);
	 $type=$this->input->post('type', TRUE);*/
	 	 if(!$cityStatus)$cityStatus=0;
        // if(!$countryCode)$countryCode=0;
...
{
    $cntry_id = $this->uri->segment(3);  
	$data['network'] = $this->generalsetting->get_network();
   // echo $cntry_id;
	$this->load->view('admin/generalsetting/get_network',$data);
} 
 //****************************************/
