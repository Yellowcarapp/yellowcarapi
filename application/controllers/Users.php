   ...
   if($priv!='')
                            ...
                            if($password!='')$data['userPass'] = md5($password);
									 ...
									 delete_cookie('adminID');
									 	 ...
									 	 redirect(site_url('Users/memberList')); 
													  ...
													  redirect(site_url('Users/Form/'.$id));
									  }
    ...
    function checkUserEmail()
    {
        ...
        if(isset($_COOKIE['adminID']) && $_COOKIE['adminID']!='')
            $wh=' userId <>'.$_COOKIE['adminID'];
        else $wh='';
        $check=$this->users->All('admins','userId','userEmail',$name,$wh);
        $arrayToJs = array();
...
function sendAdminPassword()
      { 
		 ...
		 if($this->uri->segment(3)=='ajax')
		  ...
		  $data['websitesetting']=$this->users->webSettings();
	      if($this->session->userdata('id'))
			   {
			        ...
			        $data['Error']='1';
 					$this->load->view('forget_password',$data); 
					  ...
					  $this->db->or_where('userEmail',$username);
					  ...
					  //print_r($userData); echo count($userData);exit();
 					  if(count($userData)>0)
					     {    $data['Error']='2';
					          //==Send Message
							  $data['setting'] = $this->users->webSettings();
							  $webSiteTitle=$data['setting']['SiteTitle'];
							  $webSiteEmail=$data['setting']['email'];
							  $to=$userData['userEmail'];
 							  //=====
									...
									$msg.='<br> Your Email&nbsp; : '.$userData['userEmail'];
