...
<form action="<?=site_url('admin/Category/SaveFeaturePages'); ?>" method="post" id="settingForm" >
...
<?php if(isset($page['id'])){ ?>						
						...
						<p class="button-height inline-label">
							<label for="validation-required" class="label">Page Name</label>
							<input type="text" name="title_en" id="title_en" class="input validate[required]" value="<?php if(isset($page['title'])) echo $page['title']; ?>" data-tooltip-options='{"position":"right"}' style="text-align:left;direction:ltr" >
						</p>  					
...
howMany += 1;	
$("#category").show();
$("#category").append('	<p class="button-height inline-label" id="cat'+howMany+'"><label for="validation-required" class="label">Page Name</label><input type="text" name="title_en[]" id="title_en" class="input validate[required]" data-tooltip-options="{"position":"right"}" style="text-align:left;direction:ltr"><img src="<?=$this->config->item('assets_path')?>design/image/remove.png" style="cursor:pointer" id="add_candidates" onclick="remove_category('+howMany+');">	</p>  ');
