<? if($repType==1){
    ...
    $title='Request per driver'; $subTitle='Sum of request(s)/driver';
}?>
              <!-- AREA CHART -->
                ...
                <div class="box-header with-border">
                  ...
                  <div class="box-tools pull-right">
                    ...
                    <button class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
                       <button class="btn btn-box-tool" data-widget="print" onclick="$.print('#areaChart');"><i class="fa fa-print"></i></button>
                  </div>
                    <h3 class="box-title" style="position:absolute;left:10px;"><?=$title?></h3>
                </div>
                <div class="box-body">
                  <div class="chart">
       ...
       if($tenRequest==1){?>
         ...
         <? }else if ($tenRequest==2){?>
            ...
            <? }else if ($tenRequest==3){?>
             labels[<?= $m ?>]="<?= $orders[$m]['driverFName'].' '.$orders[$m]['driverLName'].' MOBILE('.$orders[$m]['driverMobile'].')'?>"
             <? }?>
             <? }else if($repType==2){  ?>
            labels[<?= $m ?>]="<?= $orders[$m]['Timing']?>"
             <? }else if($repType==3){  ?>
      labels[<?= $m ?>]="<?= $orders[$m]['Range'].' '.$orders[$m]['name']?>"
...
<script>
        ...
        $('#report2Div').load('<?= site_url('admin/Reports/printSChartReport') ?>',data);
