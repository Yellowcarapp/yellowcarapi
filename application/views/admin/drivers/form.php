          ...
          <div class="row">
            ...
            <div class="col-md-12">
              ...
              <div class="box box-warning">
                ...
                <div class="box-body">
<form role="form" action="<?=site_url('Drivers/SaveNetwork')?>" method="post" name="formID" class="formID" id="formID" onsubmit="checkSubmit()">
<input type="hidden" name="pageid" id="pageid" value="<?php if(isset($page['network_id'])) echo $page['network_id'] ?>" />                                
  <input type="hidden" name="userId" id="userId" value="<?php if(isset($page['userId'])) echo $page['userId'] ?>" />                                
                      ...
                      <div class="form-group"><!--onblur="checkNameUique(this.value)"-->
                     <label class="full_width"><?=lang('Name')?> </label>  
 <?php if(isset($page['network_id'])) $val='ajax[ajaxNetworkCall]'; else $val='ajax[ajaxNetworkCall2]'; ?>
<input type="text" class="form-control width_size" data-validation-engine="validate[required,<?=$val?>]"
    ...
    data-errormessage="this field is required!"  name="network_name" id="network_name" value="<?php if(isset($page['network_name'])) echo $page['network_name']; ?>"  />
                    ...
                    <div class="form-group">
                      ...
                      <select class="width_size form-control validate[required]" name="network_country" id="network_country" onchange="var arr=this.value; var id=arr.split('_');$('#city_Load').load( '<?= base_url('generalsetting/get_cities/') ?>/'+id[0]);"  > 
...
console.log(response)
  ...
  var p="<img src='<?=base_url()?>resources/uploads/files/small/"+response.newFileName+"' width='70'>";
   ...
   <?php 
    ...
    //********************************//
    function checkNameUique()
    {var val=$('#network_name').val();
        console.log(val)
        $.post('<?= base_url('Drivers/check_network') ?>', { name: val }, function( data ){
            console.log(data);
            if(data==1)  { var a= '1';//return a;
                         }
            else if(data==0) {
                alert("<?=lang('network_name_error')?>"); 
                var a= '0';
             // return a;   
            }
  ...
  function checkSubmit() {
     var item= checkNameUique();
            setTimeout(function(){ if(item) 
            {alert(" HHHHHHH "+item);}
       else alert('UUUUUUUUUUUU'+item);},300000)
