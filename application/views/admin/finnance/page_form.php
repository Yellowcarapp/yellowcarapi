        ...
        <section class="content">
            ...
            <div class="col-md-12">
              ...
              <div class="box box-warning">
                ...
                <div class="box-body">
 <form action="<?=site_url('Finnance/SaveFinnance')?>" method="post" id="settingForm">
...
<input type="text"  name="refercode" id="refercode" value="" onchange="getDriverbalance()" />
    ...
    if(isset($finnanceMsg)) {
        echo $finnanceMsg; 
        if($finnanceMsg!='0') {
            $url=site_url('finnance/balanceDetails');
            $display='block';
                      ...
                      <tr>
                       ...
                       <td><?=$pages['driverFName'].' '.$pages['driverLName'];?></td>
                        ...
                        <td><div class="timeline-footer">
                          ...
                          <a class="btn btn-primary btn-xs" href="<?=site_url('finnance/finnanceForm/'.$pages['acc_id']); ?>"> 
                          <i class="fa fa-pencil"></i> <?=lang('Edit')?></a>
                            
                         <a class="btn btn-danger btn-xs jConfirmTwo" href="<?=site_url('finnance/delete/'.$pages['acc_id']); ?>" >
      ...
      autoclose: true,
        ...
        getDriverbalance=function()
        {
           // console.log( $('#refercode').val())
            $('#modal').attr('href','<?=site_url('finnance/balanceDetails')?>'+'/'+$('#refercode').val())
            $('#retFinn').load('<?=site_url('finnance/get_balance')?>',{id:$('#refercode').val()});
        }
        reloadComment=function(val)
        {
            console.log(val);
            
             $('#commSpan').load('<?=site_url('finnance/get_comment')?>',{id:val});
        }
</script>