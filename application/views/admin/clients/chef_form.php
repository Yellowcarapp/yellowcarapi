...
<form action="<?=site_url('Users/UpdateChef')?>" method="post" id="settingForm">
...
<input id="chef_month" name="chef_month" type="radio" value="1"  <?php if(isset($page['kit_chef_month'])&&$page['kit_chef_month']==1) {  ?>  checked="checked" <?php } ?>> نعم
 					...
 					<a href="<?=base_url('users/Members'); ?>" class="button blue-gradient">رجوع</a>
					<button type="submit" class="button blue-gradient">تأكيد</button>	
					<a href="<?=base_url('users/DeleteClient/'.$page['user_id']); ?>" class="button blue-gradient">حذف</a>
					</div> 
