<?php
         ...
         $uploaddirRealName='files/'.date('Y').'_'.date('m').'/';
         $uploaddir='../uploads/files/'.date('Y').'_'.date('m');
		 $dirthums='../uploads/thums/files/'.date('Y').'_'.date('m');
		 $do_not_createThumbsWaterMark=$_REQUEST['do_not_createThumbsWaterMark'];
  			 if(!file_exists($uploaddir) and !is_dir($uploaddir)) 
  			    {
   				    mkdir($uploaddir, 0777);
				  if(!file_exists($dirthums) and !is_dir($dirthums)) 
  			         {
				      mkdir($dirthums,0777);    
				     }
				} 
		...
		if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $file)) { 
		 echo $uploaddirRealName.$realname; 
		  //$creatThum=@createThumbs( '../uploads/', '../uploads/thums/', '150',$realname);
			if(!$do_not_createThumbsWaterMark)
			 {
			 $creatThum=createThumbsWaterMark($uploaddir.$realname,$realname,$uploaddir,'0');
			            @createThumbsWaterMark($uploaddir.$realname,$realname, $dirthums.'/','1');
			 }				
		 } else {
			echo "error";
		 }
  ...
  if($img)
   {
		...
		/*
			$thumb->txt_watermark='Water Mark Word';	    // [OPTIONAL] set watermark text [RECOMENDED ONLY WITH GD 2 ]
